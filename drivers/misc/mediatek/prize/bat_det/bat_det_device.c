...
/* prize added by chenjiaxi, bat det input dev, 20220224-start */
struct input_dev *bat_det_input_dev;
/* prize added by chenjiaxi, bat det input dev, 20220224-end */
...
static void bat_det_eint_handler(unsigned long data)
{
   ...
   if(cur_bat_det_status)
   {
   	BAT_DET_DEBUG("[bat_det_dev]:BAT_DET_NO \n");
		/* prize added by chenjiaxi, bat det input dev, 20220224-start */
        input_report_key(bat_det_input_dev, 0x179, 1);
        input_sync(bat_det_input_dev);
		/* prize added by chenjiaxi, bat det input dev, 20220224-end */
   ...
   else
   {
   	BAT_DET_DEBUG("[bat_det_dev]:BAT_DET_OK \n");
		/* prize added by chenjiaxi, bat det input dev, 20220224-start */
        input_report_key(bat_det_input_dev, 0x179, 0);
        input_sync(bat_det_input_dev);
		/* prize added by chenjiaxi, bat det input dev, 20220224-end */
...
#endif
	/* prize added by chenjiaxi, bat det input dev, 20220224-start */
    bat_det_input_dev = devm_input_allocate_device(&pdev->dev);
    bat_det_input_dev->name = "bat-det-input";
    bat_det_input_dev->dev.parent = &pdev->dev;
    bat_det_input_dev->evbit[0] = BIT_MASK(EV_KEY);
    bat_det_input_dev->keybit[BIT_WORD(0x179)] = BIT_MASK(0x179);
    ret = input_register_device(bat_det_input_dev);
    if (ret) {
		BAT_DET_DEBUG("register input device failed (%d)\n", ret);
		return ret;
	}
    /* prize added by chenjiaxi, bat det input dev, 20220224-end */
    return 0;
...
static int bat_det_remove(struct platform_device *dev)	
{
	...
	/* prize added by chenjiaxi, bat det input dev, 20220224-start */
    input_unregister_device(bat_det_input_dev);
	/* prize added by chenjiaxi, bat det input dev, 20220224-end */
