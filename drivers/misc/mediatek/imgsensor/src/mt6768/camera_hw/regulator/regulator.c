...
struct REGULATOR_CTRL regulator_control[REGULATOR_TYPE_MAX_NUM] = {
	...
	{"vcamd"},
	{"vcamio"},
	{"vcamaf"},// prize add by linchong 20210701
};
...
void  regulator_afvdd_set(int status) {
	...
	if(g_AFVDD_pregulator) {
		pr_debug("regulator afvdd enable status (%d)\n", status);
		if (status) {
			ret = regulator_enable(g_AFVDD_pregulator);
			pr_debug("regulator afvdd enable (%d)\n", ret);
		} else  {
			ret = regulator_disable(g_AFVDD_pregulator);
			pr_debug("regulator afvdd disable (%d)\n", ret);
		}
	}
	else {
	...
	enum IMGSENSOR_HW_PIN_STATE pin_state)
{
	...
	if (pin > IMGSENSOR_HW_PIN_MAX_NUM   ||
