...
static uint8_t jd9365t_GetSectionNum(uint32_t start_addr)
{
	uint8_t rbuf;

	if (jd9365t_ReadRegSingle(start_addr, &rbuf) < 0) {
		JD_E("%s: Get section number fail\n", __func__);
		rbuf = 0;
	}
...
static void jd9365t_ReadSectionInfo(bool reinit_config)
{
	...
	if (jd9365t_GetSectionFirstValueStatus(50) == true) {
		...
		if (jd9365t_GetSectionNum(g_jd_section_info.dsram_num_start_addr) == JD9365T_DSRAM_FW_PAYLOAD + 1) {
			jd9365t_GetSectionInfo(g_jd_section_info.dsram_section_info_start_addr,
									JD9365T_DSRAM_FW_PAYLOAD + 1, &dsram_info_content[0]);
		} else {
			/* FW dsram section was out of sync with driver */
			JD_E("%s: Please to compare FW and Driver dsram number\n", __func__);
		}

		if (jd9365t_GetSectionNum(g_jd_section_info.esram_num_start_addr) == JD9365T_ESRAM_COORDINATE_REPORT + 1) {
			...
			/* FW esram section was out of sync with driver */
			JD_E("%s: Please to compare FW and Driver esram number\n", __func__);
		}

		/* 2.1 Dump section info */
		for (i = 0; i <= JD9365T_DSRAM_FW_PAYLOAD; i++) {
		...
		if (ReCode < 0) {
			...
			g_jd9365t_chip_info.dsram_host_addr.mpap_diff_array = 0x200055DC;
#endif
...
static int jd9365t_HostWritePram(uint32_t WriteAddr, uint8_t *pFileData, uint32_t FileSize)
{
	...
	for (i = 0; i < MoveInfoNumber; i++) {
		...
		buffer = kzalloc(MoveInfo[i].fl_len * sizeof(uint8_t), GFP_KERNEL);
		memcpy(buffer, pFileData + MoveInfo[i].fl_st_addr, MoveInfo[i].fl_len);

		ReCode = jd9365t_WritePram(MoveInfo[i].to_mem_st_addr, buffer, MoveInfo[i].fl_len);
		if (ReCode < 0) {
		...
		softcrc = JD9365T_CRC_INFO_CRC_INITIAL_VALUE;
		jd9365t_GetDataCRC16(buffer, MoveInfo[i].fl_len, 0, MoveInfo[i].fl_len, &softcrc, JD9365T_CRC_INFO_PolynomialCRC16);
...
static void module_jd9365t_read_fw_ver(void)
{
    ...
    #if defined(CONFIG_PRIZE_HARDWARE_INFO)
		sprintf(current_tp_info.chip, "FW_CID_VER: %08x",pjadard_ic_data->fw_cid_ver);
		sprintf(current_tp_info.id, "JD%04XTD", JD9365T_ID);
	   	sprintf(current_tp_info.more,"touchpanel");
    #endif
	printk("FW_VER: %08x\n", pjadard_ic_data->fw_ver);
	printk("FW_CID_VER: %08x\n", pjadard_ic_data->fw_cid_ver);
}
