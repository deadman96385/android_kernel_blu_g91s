...
static void sgm41512_inform_psy_dwork_handler(struct work_struct *work)
{
	...
	sgm41512_switch_dpdm_event_detection(chip,!charger_exist);
}
/*
...
static irqreturn_t sgm41512_irq_handler(int irq, void *data)
{
	...
	ret = sgm41512_i2c_read_byte(chip,SGM41512_REG_STATUS,&evt[SGM41512_ST_STATUS]);
	if (ret < 0) {
		dev_notice(chip->dev, "%s read evt fail(%d)\n", __func__, ret);
		goto out;
	}
	
	ret = sgm41512_i2c_read_byte(chip,SGM41512_REG_STATUS + SGM41512_ST_FAULT,&evt[SGM41512_ST_FAULT]);
	if (ret < 0) {
		dev_notice(chip->dev, "%s read evt fail(%d)\n", __func__, ret);
		goto out;
	}
	ret = sgm41512_i2c_read_byte(chip,SGM41512_REG_STATUS + SGM41512_ST_STATUS2,&evt[SGM41512_ST_STATUS2]);
	if (ret < 0) {
		dev_notice(chip->dev, "%s read evt fail(%d)\n", __func__, ret);
		goto out;
	}
...
*/	
	pr_err("gezi %s STATUS REG DUMP 0x%x,0x%x,0x%x\n",__func__,evt[0],evt[1],evt[2]);
			...
			const struct i2c_device_id *id)
{
	...
	if (ret < 0)
		dev_notice(chip->dev, "%s reset register fail(%d)\n",
				      __func__, ret);
	/*prize add by zhaopengge --for ic reset 20211108  start*/				  
	msleep(100);
	pr_err("gezi-----sgm41512------delay 100 ms\n");
	/*prize add by zhaopengge --for ic reset 20211108  end*/	
