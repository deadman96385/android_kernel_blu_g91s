			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	printk("%s(), pzp status %d\n", __func__, enable);
	AudDrv_GPIO_EXTAMP_Select(enable,3);
	return 0;
}
*/
				       ...
				       int event)
{
	...
	//printk("pzp %s() event%d = %d\n", __func__,  event);
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
	//printk("p22zp %s() = %d\n", __func__, SND_SOC_DAPM_POST_PMU);
	//prize-spk amp gpio control-pengzhipeng-20210220-start
		AudDrv_GPIO_EXTAMP_Select(1,2);
	//prize-spk amp gpio control-pengzhipeng-20210220-end
		/* spk amp on control */
		break;
	case SND_SOC_DAPM_PRE_PMD:
	//printk("p111zp %s() = %d\n", __func__, SND_SOC_DAPM_PRE_PMD);
	//prize-spk amp gpio control-pengzhipeng-20210220-start
		AudDrv_GPIO_EXTAMP_Select(0,2);
	//prize-spk amp gpio control-pengzhipeng-20210220-end
		/* spk amp off control */
		break;
	default:
